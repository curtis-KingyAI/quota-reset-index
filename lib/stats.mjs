...
export function stats() {
  ...
  return {
    ...
    /** How many CURRENT records cite each host. Tells a reader which sources the ledger leans on. */
    hosts: (() => {
      const counts = {};
      for (const r of current) {
        const hosts = new Set();
        for (const e of r.evidence) {
          try {
            hosts.add(new URL(e.url).hostname.replace(/^www\./, ''));
          } catch {
            /* a malformed url is caught by validation, not here */
          }
        }
...
/**
 * The RUNBOOK's live figures.
 *
 * ⚠️ RUNBOOK PROSE MUST NOT CARRY LIVE NUMBERS. It went stale four times in one day,
 * and the last time was a figure written ONE COMMIT EARLIER: "61 of 64 evidence URLs
 * now carry a capture" became wrong the moment a new record added two URLs. The
 * document teaching people to be careful was the least accurate in the repository.
 *
 * So the split is: PROSE carries the RULE, this block carries the STATE. A number
 * describing what the ledger currently holds belongs here. A number describing what
 * happened once — "24 of 29 records", "21 of 41 corrections were wrong" — is history
 * and stays in the prose, because regenerating it would falsify the record of an
 * event that is over.
 */
export function renderRunbookStats(s = stats()) {
  const top = Object.entries(s.hosts)
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
    .slice(0, 5)
    .map(([h, n]) => `\`${h}\` ${n}`)
    .join(' · ');
  return [
    ...
    '|---|---|',
    `| Records | ${s.records.current} current, ${s.records.superseded} superseded |`,
    `| Evidence URLs archived | **${s.evidence.archivedUrls} of ${s.evidence.uniqueUrls}** — verify with \`npm run archive:verify\` |`,
    `| Records with no \`field_support\` | ${s.fieldSupport.recordsWithNone} — they predate the field |`,
    `| Most-cited sources | ${top} |`,
  ].join('\n');
}
