...
// §4.2 AMENDED 2026-07-26 (operator-approved): `telemetry` STRUCK from the
// confirming set. Telemetry can establish that an account's quota changed. It
// cannot establish that the vendor GRANTED anything — no channel, supported or
// otherwise, distinguishes a vendor-wide grant from a scheduled rollover. It
// would therefore confirm a proposition this ledger does not record. No record
// relied on it, so nothing was invalidated by the change.
const CONFIRMING_EVIDENCE = new Set(['vendor_post', 'status_page']);
...
export function validateEntries(entries, schema) {
  ...
  for (const { path, raw } of entries) {
    ...
    // effects[] (added 2026-07-26) must agree with the sealed `kind` field.
    // kind cannot be removed — it is sealed on every pre-2026-07-26 record — so
    // the two must be kept consistent rather than allowed to drift apart.
    if (Array.isArray(rec.effects) && rec.effects[0] !== rec.kind) {
      err(path, 'effects', `effects[0] is "${rec.effects[0]}" but kind is "${rec.kind}"; the primary effect must match kind`);
    }

    // field_support (added 2026-07-26) must agree with the values it describes.
    // "unestablished" is only honest if the field actually holds nothing — otherwise
    // a record would claim a value and simultaneously disclaim it.
    const fs = rec.field_support ?? {};
    const emptyish = {
      trigger: rec.trigger === 'unknown',
      scope_windows: rec.scope.windows.length === 0,
      scope_plans: rec.scope.plans.length === 0,
      scope_partial: rec.scope.partial === null,
    };
    for (const [field, support] of Object.entries(fs)) {
      if (support === 'unestablished' && !emptyish[field]) {
        err(
          path,
          `field_support.${field}`,
          `marked "unestablished" but the field holds a value. Clear it (trigger -> "unknown", ` +
            `arrays -> [], scope.partial -> null) or mark it "inferred" instead.`,
        );
      }
      if (support === 'attested' && emptyish[field]) {
        err(path, `field_support.${field}`, `marked "attested" but the field is empty — nothing is being attested`);
      }
    }
  ...
  // links[] must resolve, and must not point at the record itself.
  for (const { path, rec } of records) {
    for (const [i, link] of (rec.links ?? []).entries()) {
      if (link.id === rec.id) {
        err(path, `links.${i}.id`, `record links to itself with relation "${link.relation}"`);
        continue;
      }
      if (!byId.has(link.id)) {
        err(path, `links.${i}.id`, `${link.relation} points at "${link.id}", which is not an existing record`);
      }
    }
  }
