...
export function bestCapture(entry) {
  ...
  for (const a of attempts) {
    if (a.method !== 'verify' || !a.archive_url) continue;
    const prev = latestVerdict.get(a.archive_url);
    if (!prev || (a.checked_at ?? '') >= (prev.checked_at ?? '')) latestVerdict.set(a.archive_url, a);
  }
