...
test('running the generator twice changes nothing the second time', () => {
  // If this fails, CI's reproducibility check flaps on every run and someone
  // eventually deletes it. That would be worse than the counts it replaced.
  const first = buildDocs({ write: false });
  const second = buildDocs({ write: false });
  assert.deepEqual(
    first.results.map((r) => r.file),
    second.results.map((r) => r.file),
  );
  for (const r of second.results) {
    assert.equal(r.changed, false, `${r.file} still changes on a second pass — generator is not idempotent`);
  }
});
