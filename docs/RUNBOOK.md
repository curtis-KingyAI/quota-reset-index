...
<!-- generated:runbook-stats -->
| the ledger right now | |
|---|---|
| Records | 29 current, 20 superseded |
| Evidence URLs archived | **61 of 66** — verify with `npm run archive:verify` |
| Records with no `field_support` | 9 — they predate the field |
| Most-cited sources | `community.openai.com` 12 · `explainx.ai` 9 · `minimaxir.com` 9 · `codex-resets.com` 5 · `codexreset.org` 3 |
<!-- /generated:runbook-stats -->

⚠️ **The table above is generated by `npm run build`; everything else in this file is written by hand.**
That split is deliberate. Prose here carries the RULE; the table carries the STATE. A figure describing
what the ledger currently holds goes in the table — inline numbers in this document went stale four
times in a single day, once within one commit of being written. A figure describing something that
*happened once* — "24 of 29 records", "21 of 41 corrections were wrong" — stays in the prose, because
regenerating it would falsify the record of an event that is over.
...
- If a source is important and fragile, archive it: `npm run archive -- --save`, and verify
  periodically with `npm run archive:verify`. Current coverage is in the table at the top of this
  file — **do not quote a figure inline here**; the one that used to sit in this bullet was wrong
  twice, the second time one commit after it was written. A single HTTP 404 from Wayback is NOT
  evidence a capture is gone; it takes two, because the archive returns them transiently under load.
...
> ids. Do not misclassify it — several current records rest on it, and the count is in the table at
> the top of this file rather than here, where it would go stale.
...
> Some current records carry **no** `field_support` at all — the count is in the table at the top of
> this file. They predate the field. Absent means
